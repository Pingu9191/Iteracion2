...
int main(int argc, char** argv) {
  ...
  if (all || test == 31) test3_space_add_object();
  if (all || test == 32) test1_space_delete_object();
  if (all || test == 33) test2_space_delete_object();
  if (all || test == 34) test3_space_delete_object();
  if (all || test == 35) test1_space_create_gdesc();
  if (all || test == 36) test2_space_create_gdesc();
  if (all || test == 37) test1_space_destroy_gdesc();
  if (all || test == 38) test2_space_destroy_gdesc();
  if (all || test == 39) test3_space_destroy_gdesc();
  if (all || test == 40) test1_space_get_n_objects();
  if (all || test == 41) test2_space_get_n_objects();
  if (all || test == 42) test3_space_get_n_objects();
  if (all || test == 43) test1_space_set_gdesc();
  if (all || test == 44) test2_space_set_gdesc();
  if (all || test == 45) test3_space_set_gdesc();
...
void test1_space_set_object() {
  Space *s;
  s = space_create(1);
  PRINT_TEST_RESULT(space_set_object(s,set_create()) == OK);
  space_destroy(s);
}

void test2_space_set_object() {
  Space *s = NULL;
  PRINT_TEST_RESULT(space_set_object(s,set_create()) == ERROR);
}
...
void test1_space_get_object() {
  Space *s;
  s = space_create(1);
  space_set_object(s,set_create());
  space_add_object(s, TRUE);
  PRINT_TEST_RESULT(set_idInSet(space_get_object(s), TRUE)==TRUE);
  space_destroy(s);  
}
...
void test1_space_delete_object() {
  ...
  PRINT_TEST_RESULT(space_delete_object(s, 1) == OK);
  space_destroy(s);
}

void test2_space_delete_object() {
  Space *s = NULL;
  PRINT_TEST_RESULT(space_delete_object(s, TRUE) == ERROR);
}
...
void test3_space_delete_object() {
  ...
  s = space_create(1);
  space_add_object(s, 1);
  PRINT_TEST_RESULT(space_delete_object(s, 2) == ERROR);
  space_destroy(s);
}
...
void test1_space_destroy_gdesc() {
  Space *s = NULL;
  PRINT_TEST_RESULT(space_remove_gdesc(s) == ERROR);
}
...
void test1_space_get_n_objects() {
  Space *s = NULL;
  PRINT_TEST_RESULT(space_get_n_objects(s) == NO_ID);
}
...
void test1_space_set_gdesc(){
  Space *s= NULL;
  char **o=NULL;
  PRINT_TEST_RESULT(space_set_gdesc(s, o) == ERROR);
}
...
void test3_space_set_gdesc(){
  ...
  s = space_create(1);
  char **o = space_create_gdesc(s);
  char **u = space_create_gdesc(s);
  space_set_gdesc(s, u);
  PRINT_TEST_RESULT(space_set_gdesc(s, o) == OK);
  space_destroy(s);
}
